...
from .model import Catalogue, FootprintSet, MetadataType, HiPS
import csv
import json
import os.path
import tornado.web
...
class ESASkyWidget(widgets.DOMWidget):
    ...
    def __init__(self, lang="en", enable_logs=True, log_level=logging.DEBUG):
        super().__init__()
        ...
        available_lang = ["en", "es", "zh"]
        if lang.lower() in available_lang:
            self._view_language = lang
        else:
            available_lang_str = ", ".join(available_lang)
            raise ValueError(
                f"Wrong language code used. Available languages are {available_lang_str}"
            )
    ...
    def _check_server_version(self):
        version_resp = requests.get("https://sky.esa.int/esasky-tap/version")
        ...
        if match:
            ...
            if server_version > self._intended_server_version:
                warning_message = (
                    "The ESASky server has been updated since your installation of pyESASky.\n\n"
                    "Some commands might malfunction. Please upgrade your installation if you experience any issues.\n\n"
    ...
    def _handle_comm_message(self, type, content):
        logger.debug(f"recieved comm message of type {type}")
        if type == const.MESSAGE_TYPE_DOWNLOAD:
            url = content.get("url")
            response = requests.get(url.strip(), allow_redirects=True, timeout=10)
            if response.status_code == 200:
                filename: str = self._getFilenameFromContentDisposition(response)
                self.fc.reset(filename=filename)
                self.fc.register_callback(
                    lambda fc: self._saveFile(fc, response.content)
                )
                self.tt.value = "Downloading " + filename
                self.tt.notify_change("test")

    def _saveFile(self, fileChooser: FileChooser, content: bytes):
        if (
            fileChooser.selected is not None
        ):  # asksaveasfile is "None" when Dialog is closed with "cancel"
            with open(fileChooser.selected, "wb") as f:
                f.write(content)
