...
class ESASkyWidget(widgets.DOMWidget):
    ...
    def __init__(self, lang='en'):
        ...
        if lang.lower() in availableLanguages:
            ...
            raise EnvironmentError(
                "Wrong language code used. Available are " + str(availableLanguages).strip('[]'))
    ...
    def overlayFootprintsFromCSV(self, pathToFile, csvDelimiter, footprintSetDescriptor):
        ...
        footprintSet = FootprintSet(footprintSetDescriptor.getDatasetName(
        ), 'J2000', footprintSetDescriptor.getHistoColor(), footprintSetDescriptor.getLineWidth())
                            ...
                            if len(footprintSetDescriptor.getMetadata()) > 0:
                                ...
                                while j < len(footprintSetDescriptor.getMetadata()):
                                    if footprintSetDescriptor.getMetadata()[j].getLabel() == columns[i]:
                                        found = True
                                        currMetadata['name'] = footprintSetDescriptor.getMetadata()[
                                            j].getLabel()
                                        currMetadata['value'] = row[i]
                                        currMetadata['type'] = footprintSetDescriptor.getMetadata()[
                                            j].getType()
    ...
    def overlayFootprintsFromAstropyTable(self, footprintSetDescriptor, table):
        ...
        astropyFootprintSet = FootprintSet(footprintSetDescriptor.getDatasetName(
        ), 'J2000', footprintSetDescriptor.getHistoColor(), footprintSetDescriptor.getLineWidth())
    ...
    def overlayCatalogueFromCSV(self, pathToFile, csvDelimiter, catalogueDescriptor, cooFrame):
        ...
        catalogue = Catalogue(catalogueDescriptor.getDatasetName(
        ), cooFrame, catalogueDescriptor.getHistoColor(), catalogueDescriptor.getLineWidth())
        ...
        with open(pathToFile) as csv_file:
            ...
            for row in csv_reader:
                if line_count == 0:
                            ...
                            if len(catalogueDescriptor.getMetadata()) > 0:
                                ...
                                while j < len(catalogueDescriptor.getMetadata()):
                                    if catalogueDescriptor.getMetadata()[j].getLabel() == columns[i]:
                                        found = True
                                        currMetadata['name'] = catalogueDescriptor.getMetadata()[
                                            j].getLabel()
                                        currMetadata['value'] = row[i]
                                        currMetadata['type'] = catalogueDescriptor.getMetadata()[
                                            j].getType()
                                        break
                                    j += 1
                            elif not found:
                    ...
                    catalogue.addSource(currName, currRaDeg,
                                        currDecDeg, currId, currDetails)
                    line_count += 1
            print(f'Processed {line_count} lines.')
    ...
    def _readProperties(self, url):
        ...
        if not url.startswith('http'):
            ...
            try:
                ...
                print(
                    url + " not found or missing properties file.\n Did you mean http://" + url)
                raise (fnf_error)
        else:
    ...
    def browseHips(self):
        ...
        columns = ["ID", "obs_title", "moc_order",
                   "moc_sky_fraction", "em_min", "em_max", "hips_service_url"]
