...
var ESASkyJSView = widgets.DOMWidgetView.extend({
	...
	render: function () {
		...
		//div.innerHTML = "<iframe id='esaskyFrame' width='100%' height='800' style='border: none;' src='http://localhost:8080/test?log_level=DEBUG'</iframe>";
	...
	handle_custom_message: function (msg) {
		...
		switch (msg['event']) {
			case 'setFoV':
				console.log("FoV changed " + msg['fovDeg']);
				document.getElementById('esaskyFrame').contentWindow.ESASkyAPI.setFoV(msg['fovDeg']);
				break;

			case 'goToTargetName':
				console.log("Targetname changed  " + msg['targetname']);
				document.getElementById('esaskyFrame').contentWindow.ESASkyAPI.goToTargetName(msg['targetname']);
				break;

			case 'setHiPSColorPalette':
				console.log("ColorPalette changed " + msg['colorPalette']);
				document.getElementById('esaskyFrame').contentWindow.ESASkyAPI.setHiPSColorPalette(msg['colorPalette']);
				break;
			...
			case 'changeHiPS':
				...
				console.log(msg);
				console.log("HiPS URL "+msg['content']);
				return document.getElementById('esaskyFrame').contentWindow.ESASkyAPI.setHiPS(msg['content'])
			...
			case 'addJwst':
				...
				console.log(msg);
				return document.getElementById('esaskyFrame').contentWindow.ESASkyAPI.addJwstAPI(msg['instrument'],msg['detector'],msg['showAllInstruments']);

			case 'addJwstWithCoordinates':
				console.log('addJwstWithCoordinates event captured!');
				console.log(msg);
				return document.getElementById('esaskyFrame').contentWindow.ESASkyAPI.addJwstWithCoordinates(msg['instrument'],msg['detector'],msg['showAllInstruments'],msg['ra'], msg['dec'], msg['rotation']);

			case 'closeJwstPanel':
				console.log('closeJwstPanel event captured!');
				console.log(msg);
				return document.getElementById('esaskyFrame').contentWindow.ESASkyAPI.closeJwstPanel();

			case 'openJwstPanel':
				console.log('openJwstPanel event captured!');
				console.log(msg);
				return document.getElementById('esaskyFrame').contentWindow.ESASkyAPI.openJwstPanel();
				
			case 'clearJwstAll':
				console.log('clearJwst event captured!');
				console.log(msg);
				return document.getElementById('esaskyFrame').contentWindow.ESASkyAPI.clearJwst();
