...
var ESASkyJSView = widgets.DOMWidgetView.extend({
	...
	render: function () {
		...
		window.addEventListener("message",function(e){
			...
			if("message" in data){
				_this.model.set('_messageSync',data["message"]);
				_this.model.save_changes();	
			}else {
				_this.model.set('callback',data);
				_this.model.set('_messageSync',"Reponse received in variable \"callback\" and can be accessed in a subsequent cell");
				_this.model.save_changes();	
			}
		});
